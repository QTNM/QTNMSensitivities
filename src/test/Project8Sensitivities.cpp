...
int main(int argc, char *argv[]) {
  ...
  auto *mg90CL = new TMultiGraph(
      "mg90CL",
      "arXiv:1309.7093; Effective volume [m^{3}]; 90% CL mass limit [eV]");
  auto *mgSigma = new TMultiGraph("mgSigma",
                                  "arXiv:1309.7093; Effective volume [m^{3}]; "
                                  "#sigma_{m_{#beta}^{2}} [eV^{2}]");
  ...
  systs1.push_back(GasScattering(nMol1, bGraph, false));
  ...
  systs2.push_back(GasScattering(nMol2, bGraph, false));
  ...
  systs3.push_back(GasScattering(nMol3, bGraph, false));
  ...
  for (auto &s : systs1) {
    cout << s.GetName() << "\t" << s.GetWidth() << " eV\t" << s.GetUnc()
         << " eV\n";
  ...
  systsA.push_back(MolecularTritium());
  systsA.push_back(
      SystematicEffect(energyRes, energyRes * 0.01, "Energy Res. Syst"));
  systsA.push_back(GasScattering(nMolA, bGraph, false));
  systsA.push_back(ThermalBroadening(gasTempMol, false));
  TGraph *gr90CLA = new TGraph();
  SetGraphAttr(gr90CLA);
  gr90CLA->SetLineColor(kBlue);
  gr90CLA->SetTitle(
      "arXiv:2102.00594; Volume #times Efficiency #times Time [m^{3} yr]; 90% "
      "CL m_{#beta} [eV]");
  ...
  systsB.push_back(
      SystematicEffect(energyRes, energyRes * 0.01, "Energy Res. Syst"));
  systsB.push_back(GasScattering(nMolB, bGraph, true));
  systsB.push_back(ThermalBroadening(gasTempAtom, true));
  TGraph *gr90CLB = new TGraph();
  SetGraphAttr(gr90CLB);
  gr90CLB->SetLineColor(kRed);
  gr90CLB->SetTitle(
      "T, 7 #times 10^{17} m^{-3}; Volume #times Efficiency #times Time [m^{3} "
      "yr]; 90% CL m_{#beta} [eV]");
  ...
  double logExpDiff{(log10(exposureMax) - log10(exposureMin)) /
                    double(nVPnts - 1)};
  for (int n{0}; n < nVPnts; n++) {
